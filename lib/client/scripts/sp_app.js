...
var _LOGIN_COMPONENT = null;
var _LOGOUT_COMPONENT = null;
var _PAGE_NOT_FOUND_COMPONENT = null;
var _PAGE_COMPONENT = null;
var _PAGE_MAP = {};
var _LOGIN_PREREQS = {};
var _SYSTEM_PATHS = ['account_settings', 'page_not_found', 'page_editor', 'site_metadata_editor', 'site_template_saver', 'page_manager'];
var _SYSTEM_COMPONENTS = ['AccountSettings', 'SpPageNotFound', 'PageEditor', 'SiteMetadataEditor', 'SiteTemplateSaver', 'PageManager'];
var _SYSTEM_PATHS_ACCESS = ['MEMBER', null, 'SUPERADMIN', 'ADMIN', 'SUPERADMIN', 'SUPERADMIN'];
var _DYNAMIC_SYSTEM_COMPONENTS = ['TreeEditor'];
...
var SPApp = {
  ...
  init: function (env, msgs, cTemplates, appComp, loginComp, logoutComp, notFoundComp) {
    ...
    _LOGIN_COMPONENT = loginComp ? loginComp : SPComponents['SpLogin'];
    _LOGOUT_COMPONENT = logoutComp ? logoutComp : SPComponents['SpLogout'];
    _PAGE_NOT_FOUND_COMPONENT = notFoundComp ? notFoundComp : SPComponents['SpPageNotFound'];
    setupVue(appComp, loginComp, logoutComp);  
    getAppInfo();
  },
  ...
  showLoginPage: function (route) {
    if (route) {
      let r = {path: route.path, query: route.query};
      localStorage.setItem('SP_PRE_LOGIN_PATH', JSON.stringify(r));
    }
  ...
  loginFail: function(status, errorMsg /*, result*/) {
    SPApp.modal.showErrorDialog(errorMsg);
  },
...
function appInfoSuccess(respObj) {
  ...
  if (SPApp.user && SPApp.user.loginPrereqs && SPApp.user.loginPrereqs.length > 0) {
    ...
    // let currentPath = '/';
    let route = localStorage.getItem('SP_PRE_LOGIN_PATH');
    if (route) {
      route = JSON.parse(route);
      localStorage.removeItem('SP_PRE_LOGIN_PATH');
      SPApp.router.push(route);
    } 
    
    /*else if (window.location.hash) {
      currentPath = window.location.hash.substring(1);
      let basePath = currentPath.substring(1);
      if (basePath.indexOf('/') > 0) basePath = basePath.substring(0, basePath.indexOf('/'));
      SPApp.router.push(currentPath);
    }
    */
  }
  ...
  if (SPApp.user && SPApp.user.isSuperadmin) {
    SPApp.loadScript(SPApp.config.CDN_URL + '/lib/portal_editor.umd.js', 
      function() { SPPortalEditor.SPRPC = SPApp.rpc; SPPortalEditor.SPModal = SPApp.modal; SPApp.portalEditor = SPPortalEditor; });
  }
  _INITIALIZED = true;
...
function setupRoutes(siteDef) {
  ...
  rr.push({path: '/logout', component: _LOGOUT_COMPONENT});
  rr.push({path: '/login', component: _LOGIN_COMPONENT});
...
function setupVue(appComp, loginComp, logoutComp) {
  ...
  if (loginComp) Vue.component('sp-app-login', loginComp);
  if (logoutComp) Vue.component('sp-app-logout', logoutComp);
