...
var ServiceGateway = {
  ...
  async handleServiceRequest(req, res, data) { 
    ...
    if (data.method == 'init') {
      Response.internalError(res, 'Sorry! You cannot call service init method remotely.');
      return;
    }

    if (!ServiceDirectory.getService(serviceName)) {
    ...
    if (serviceName == 'SessionService') {
      ...
      try {
        ...
        await ServiceDirectory.getService('SessionService')[data.method](req, res, data);
        return;
      } catch (e) {
        console.error(e);
    ...
    let service = ServiceDirectory.getService(serviceName);
    ...
    if (service[data.method]) {
      var user = null;
      try {
        var sessionInfo = await ServiceDirectory.getService('SessionService').getSessionInfo(req, data.params);
