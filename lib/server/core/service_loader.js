...
var _CONFIG = null;
var _SYSTEM_SERVICE_NAMES = ['AppService', 'UserService', 'SiteService', 'SessionService', 'URLFetcher'];
var _SYSTEM_INTERNAL_SERVICE_NAMES = ['ObjectStore', 'Cache', 'EmailService', 'FileStore', 'TemplateService'];
var Services = {};
var InternalServices = {};
var SP = { AppError: AppError, ServiceDirectory: ServiceDirectory};
var ServiceLoader = {
  async init(config) {
    _CONFIG = config;
    ServiceDirectory.setServices([Services, InternalServices]);
    ...
    for (let i=0; i<s.length; i++) {
      if (Services[s[i]]['init'])  await Services[s[i]].init(config, SP);
    }
    ...
    for (let i=0; i<s.length; i++) {
      if (InternalServices[s[i]]['init']) await InternalServices[s[i]].init(config, SP);
    }
  ...
  async registerService(name, service) {
    ...
    if (service.init) {
      await service.init(_CONFIG, SP);
    }
    Services[name] = service;
  },

  serviceChanged(path, action) {
    if (action == 'add') ServiceLoader.addService(path);
    else if (action == 'change') ServiceLoader.updateService(path);
    else if (action == 'unlink') ServiceLoader.removeService(path);
  },
