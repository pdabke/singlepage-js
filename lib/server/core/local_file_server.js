...
const LocalFileServer = {
  init(config, buildOutput) {
    ...
    if (process.env.SP_DIST_DIR || process.env.NODE_ENV == 'production') {
      ...
      _ASSETS_BASE_DIR = path.resolve(process.env.SP_APP_BASE, 'src', 'www');
    }
    ...
    Cache = ServiceGateway.getInternalService("Cache").permanentCache;
    LocalFileServer.readIndexFile();
    
    if (process.env.NODE_ENV == 'development') {
      let chokidar = require('chokidar');
      var indexWatcher = chokidar.watch(LocalFileServer.getIndexFileLocation());
      indexWatcher.on('ready', function () {
  ...
  getIndexFileLocation() {
    return Util.isFile(path.resolve(_ASSETS_BASE_DIR, 'index.html')) ?
      path.resolve(_ASSETS_BASE_DIR, 'index.html') :
  ...
  async handleSPFileRequest(res, relPath) {
    ...
    if (relPath.startsWith('components/')) {
      ...
      if (_BUILD_OUTPUT) {
        Response.sendFile(res, fs.readFileSync(path.resolve(process.env.SP_HOME, 'dist', 'components', comp)), relPath); 
      } else {
        Response.sendFile(res, fs.readFileSync(path.resolve(_ASSETS_BASE_DIR, 'spcomponents', comp)), relPath);
      }
      return;
    } else {
      ...
      if (fs.existsSync(path.join(_ASSETS_BASE_DIR, relPath))) {
        ...
        console.error('Recived request for non-existent file: ' + relPath);
...
async function handlePortalFileRequest(req, res, fileName, ns) {
  try {
    ...
    var rawSiteInfo = await ServiceGateway.getService('SiteService').getSiteInfo({}, {siteURL: siteUrl}, {}, ['SUPERADMIN']);
    ...
    localEnv.SITE_LOGO_URL = rawSiteInfo.logoURL ? rawSiteInfo.logoURL : _CONFIG.CDN_URL + "/images/app_logo_light.png";
