...
const StaticSiteService = {
  ACL: {
    ...
    saveSiteConfig: [Constants.ROLE_ADMIN, Constants.ROLE_SUPERADMIN],
  ...
  async saveSiteConfig(params) {
    if (!_SITE_INFO.siteData) _SITE_INFO.siteData = {};
    _SITE_INFO.siteData.siteConfig = params.config;
    saveSiteInfo();
  },
...
function saveSiteInfo(filePath) {
  ...
  // It's possible that the site change involved change in component or
  // page permissions so remove the access flag caching
  removeAccessFlag(_SITE_INFO.siteDef);
  //console.log(JSON.stringify(sinfo));
  if (filePath)
    fs.writeFileSync(filePath, JSON.stringify(_SITE_INFO));
  else
    fs.writeFileSync(_SITE_INFO_PATH, JSON.stringify(_SITE_INFO));
}
